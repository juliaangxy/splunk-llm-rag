...
install_compose() {
	...
	elif is_airgapped; then
		# The GitHub download fallback cannot work without internet egress.
		# The NVIDIA DLAMI ships Docker Compose; if we reach here it is missing.
		error "Docker Compose is unavailable and AIRGAPPED=true forbids the GitHub download fallback. Pre-install docker-compose-plugin (via the S3-backed dnf repo or the DLAMI) before deploying the airgapped environment."
		exit 1
	else
		local arch
		local compose_arch
