...
# Container images for the Milvus stack. In airgapped mode deploy.sh sets these
# to pre-seeded ECR URIs so nothing is pulled from public registries.
ETCD_IMAGE="${ETCD_IMAGE:-quay.io/coreos/etcd:v3.5.12}"
MINIO_IMAGE="${MINIO_IMAGE:-minio/minio:RELEASE.2024-06-11T03-13-30Z}"
MILVUS_IMAGE="${MILVUS_IMAGE:-milvusdb/milvus:v2.4.8}"
...
# Ensure the stack images are present (ECR login + pull when airgapped).
for milvus_image in "${ETCD_IMAGE}" "${MINIO_IMAGE}" "${MILVUS_IMAGE}"; do
