...
# Splunk is already installed, started (init.d + CLI in stage 06), and confirmed healthy
# by the check above. Do NOT switch management models here — converting to systemd
# mid-bootstrap is what left splunkd hung. Just ensure init.d boot-start stays enabled.
"${SPLUNK_HOME}/bin/splunk" enable boot-start -user splunk --accept-license --answer-yes --no-prompt >/dev/null 2>&1 || true
...
# A fresh install runs in the Trial group; adding licenses does NOT switch the active
# group. Explicitly activate Enterprise so Splunk uses the imported license(s), then
# restart (the active-group change only takes effect after a restart).
if splunk_run list licenser-groups 2>/dev/null | grep -Eiq '^Enterprise[[:space:]].*is_active[[:space:]]*:?[[:space:]]*1|Enterprise.*active'; then
	log "Enterprise licenser group already active"
else
	log "Activating the Enterprise licenser group (switching off the Trial group)"
	if splunk_run edit licenser-groups Enterprise -is_active 1 >/dev/null 2>&1; then
		# The active-group change needs a restart. Bounded CLI restart (init.d/CLI managed,
		# same as stage 06) so a slow start surfaces splunkd.log instead of hanging.
		log "Enterprise licenser group set active; restarting Splunk (bounded)"
		...
		wait_for_port 127.0.0.1 8089 180 || warn "splunkd not back on 8089 after restart (see splunkd.log)"
		wait_for_port 127.0.0.1 8000 180 || warn "splunkweb not back on 8000 after restart"
	else
		warn "Could not activate the Enterprise licenser group; Splunk may remain on the Trial group"
	fi
fi
