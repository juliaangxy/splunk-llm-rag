...
# --- EC2 instance-metadata (IMDSv2) + role→IP discovery -------------------------
# Used to resolve peer instances (e.g. the search head) by their SplunkAiRole tag so
# the token-metering proxy can ship metrics to a chosen Splunk instance without the IP
# being hard-coded. All best-effort: empty output on any failure, callers handle it.
imds_token() {
